...
public class AppDeviceHelper {
	...
	private static final Logger log = Logger.getLogger(AppDeviceHelper.class.getName());

	// Refer to table metadata names by byte array in the HBase API
	private static final byte[] TABLE_NAME = Bytes.toBytes("AppDevice");
	private static final byte[] COLUMN_FAMILY_NAME = Bytes.toBytes("AppDevice");
	private static final byte[] COLUMN_NAME = Bytes.toBytes("propertyMap");
	...
	public String findByKey(String keyName) {
		...
		try (Connection connection = BigtableConfiguration.connect(PROJECT_ID, INSTANCE_ID)) {
			...
			Table table = connection.getTable(TableName.valueOf(TABLE_NAME));
			log.info("find key");
		    Result getResult = table.get(new Get(Bytes.toBytes(keyName)));
		    log.info(getResult.toString());
		    value = Bytes.toString(getResult.getValue(COLUMN_FAMILY_NAME, COLUMN_NAME));
		    log.info(value);
